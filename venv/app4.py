...
API_PRED_ENDPOINT = os.getenv("REMOTE_API_PRED_ENDPOINT", "/predict")
...
app_ui = ui.page_sidebar(
    ui.sidebar(
        ...
        ui.markdown(
            "If no file is uploaded, the app will try to use the training CSV"
        ),
    ),
    ...
    def predictions_df() -> pd.DataFrame | None:
        ...
        if USE_REMOTE:
            try:
                ...
                # If remote fails and you have a local model, fall back
                if MODEL is not None:
                    print("[warn] Remote failed; falling back to local model.")
                else:
                    # No local model—surface None
                    ui.notification_show(
                        f"Remote prediction failed: {e}", duration=8, type="error"
                    )
                    return None
...
def _post_predict(payload: list[dict]) -> list[float]:
    """POST to /predict and return the probabilities list."""
    ...
    try:
        ...
        if not isinstance(probs, list):
            raise ValueError(f"Probabilities not a list: {type(probs)}")
        return probs
    except Exception as e:
        # Surface a short message in UI; also print full error to server logs
...
def remote_predict_proba(X: pd.DataFrame, features: list[str]) -> np.ndarray:
    ...
    Xp = X[features].astype(float).round(6)
    records = Xp.to_dict(orient="records")
